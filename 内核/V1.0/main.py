...
def build_packet(message, msg_type='001'):
   ...
    
    # 构建原始数据包
    raw_packet = f"\\StStStStSt {callsign} {timestamp} {msg_type} \"{message}\" \"{control}\""
    
    # 计算MD5
...
def decode_from_flac(input_file):
   ...
    
    if parsed is None:
        ...
        return None
    
    callsign, timestamp, msg_type, content, control, md5 = parsed
    
    # 验证MD5
    expected_md5 = hashlib.md5(message.split(md5)[0].strip().encode('utf-8')).hexdigest()[:8]
    if expected_md5 != md5:
   ...
    
    print(f"解码结果: 呼号={callsign}, 时间={timestamp}, 类型={msg_type}, 内容={content}, 控制={control}")
    return message
...
def build_retransmit_packet():
   ...
    
    ...
    if not match:
        ...
        return None
    
    callsign, old_timestamp, msg_type, content, control, md5 = match.groups()
    
    # 构建重传数据包
    retransmit_packet = build_packet(content, '010')
    return retransmit_packet
...
def parse_packet(packet):
   ...
    
    # 验证MD5
    expected_md5 = hashlib.md5(packet.split(md5)[0].strip().encode('utf-8')).hexdigest()[:8]
    if expected_md5 != md5:
        print(f"警告: MD5校验失败 (预期: {expected_md5}, 实际: {md5})")
    
    # 解析字段
    ...
    if not match:
        ...
        return None
    
    callsign, timestamp, msg_type, content, control = match.groups()
    
    return callsign, timestamp, msg_type, content, control, md5
...
if __name__ == "__main__":
    print("欢迎使用 FAITCC(four QAM anti-interference text communication codec)！\n内核版本：V1.0 ©2025 童顺\n你可以用任何方式使用，分发，研究，改进，回馈本软件，但是必须保留版权声明 ")
    main()
